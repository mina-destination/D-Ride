...
import { Sun, Moon, User, Menu, X, MapPin, LogOut, Globe, Bell, HelpCircle, Sparkles, Map, Handshake, Mail, ChevronRight, Compass, Wallet } from 'lucide-react';
          ...
          <>
              ...
              <div className={`profile-dropdown ${isDropdownOpen ? 'open' : ''}`}>
                ...
                <ul className="profile-dropdown-menu">
                  ...
                  <li>
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                        <Wallet size={16} /> <span>{language === 'ar' ? 'المحفظة' : 'Wallet'}</span>
                      </div>
                      <ChevronRight size={14} className="profile-menu-chevron" />
                    </Link>
                  </li>
            ...
            <li className="mobile-only">
              <Link
                to="/wallet"
                onClick={() => setIsOpen(false)}
                className={isRouteActive('/wallet') ? 'nav-link-active' : ''}
              >
                <Wallet size={16} className="nav-mobile-icon" />
